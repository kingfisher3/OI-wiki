...
## 另一种 Four russian 算法的改进

我们知道 Four russian 算法的瓶颈在块内 RMQ 问题，因此我们考虑如何优化块内 RMQ。

考虑对每一块跑一个单调递增的单调栈，然后对块内的每一个位置记录以该位置为右端点时的单调栈。预处理的时间复杂度 $O(n)$，空间复杂度 $O(n\log n)$。

对于一个在同一块内的询问 $l,r$，这个询问的答案就是 $r$ 位置记录的单调栈中，第一个大于等于 $l$ 的位置对应的数。查询的时间复杂度 $O(\log n)$。

由于块长是 $\log n$，因此我们可以用二进制状压来维护某个位置为右端点的单调栈。如果单调栈中包含块中第 $i$ 个元素，那么对应二进制表示的第 $i$ 位为 $1$，反之为 $0$。这样预处理的空间复杂度就优化到了 $O(n)$。

同理，在查询时，第一个大于等于 $l$ 的位置对应的数的位置可以表示为 $r$ 记录的二进制表示除去前 $l$ 位，第一个为 $1$ 的位。因为块长是 $\log n$，所以每个二进制表示均不超过 $2^{\log n} = n$，因此我们可以预处理出任意一种二进制表示第一个为 $1$ 的位。这样查询的时间复杂度也是 $O(1)$，预处理时间复杂度不变。

这样 Four russian 算法就被我们优化到了 $O(n)-O(1)$，并且该算法的常数较小。
